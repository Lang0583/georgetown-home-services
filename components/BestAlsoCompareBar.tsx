...
export default function BestAlsoCompareBar({ links }: { links: AlsoCompareLink[] }) {
  ...
  return (
    <div
      ...
      className="fixed bottom-0 left-0 right-0 z-[60] w-full border-t-[3px] border-primary bg-white px-6 py-4 shadow-[0_-4px_14px_rgba(0,0,0,0.12)] md:py-3"
    >
      <div className="mx-auto flex w-full max-w-5xl items-center gap-3">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-3 gap-y-3 sm:gap-x-2 sm:gap-y-2">
          <span className="shrink-0 text-sm font-semibold text-gray-900 md:text-sm">Also compare:</span>
          <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2 sm:gap-x-2">
            {links.flatMap((l, i) => [
              ...
              <Link
                key={l.href}
                href={l.href}
                className="inline-flex min-h-[48px] min-w-[44px] items-center rounded-md px-2 py-2 text-base font-bold leading-tight text-primary underline-offset-2 hover:underline sm:min-h-0 sm:min-w-0 sm:px-1 sm:py-1 sm:text-sm"
              >
                {l.label}
              </Link>,
            ])}
        ...
        <button
          ...
          className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-900 md:h-10 md:w-10"
        >
