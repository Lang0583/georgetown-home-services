          ...
          {guides.map((guide) => (
            <li key={guide.slug}>
              <Link
                ...
                className="flex h-full flex-col rounded-xl border border-ink/10 bg-surface p-5 shadow-sm transition hover:border-brand/30 hover:shadow-md"
              >
                <h2 className="text-lg font-semibold text-ink">{guide.h1.replace(/\? \(.*\)$/, "?")}</h2>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-muted">{guide.indexBlurb}</p>
